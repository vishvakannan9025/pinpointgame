...
class RoomManager {
    static DEFAULT_ROOM_ID = 'PINPOINT';
    rooms = new Map();
    socketMap = new Map();
    constructor() {
     ...
     */
    bindAdminToDefaultRoom(adminSocketId) {
     ...
     */
    joinRoom(roomId, rawName, socketId, reconnectParticipantId) {
        ...
        const participantId = crypto_1.default.randomUUID();
        const participant = {
            participantId,
            name: trimmedName,
            socketId,
     ...
     */
    kickParticipant(roomId, adminToken, participantId) {
        const room = this.getRoom(roomId);
        if (!room)
            return { success: false, error: 'Room not found.' };
        if (room.adminToken !== adminToken)
     ...
     */
    clearAllParticipants(roomId, adminToken) {
        const room = this.getRoom(roomId);
        if (!room)
            return { success: false, error: 'Room not found.' };
        if (room.adminToken !== adminToken)
        ...
        for (const p of room.participants.values()) {
            ...
            if (p.socketId)
                this.socketMap.delete(p.socketId);
        }
     ...
     */
    reconnectAdmin(roomId, adminToken, newSocketId) {
        const room = this.getRoom(roomId);
        if (!room) {
            return { success: false, error: 'Room does not exist or has ended.' };
        }
